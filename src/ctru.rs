...
impl CtruError {
    pub fn from_code(code: u32) -> CtruError {
        let description = (code & 0x3FF) as u16;
        let module = (code >> 10) as u8;
        // unknown: (err >> 18) & 7, seems to always be 0?
        let summary = ((code >> 21) & 0x3F) as u8;
        let level = (code >> 27) as u8;
...
impl Display for CtruError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            concat!(
                "Nintendo error in module {}:\n",
                "\tSummary: {}\n",
                "\tLevel: {}\n",
                "\tDescription: {}"
            ),
