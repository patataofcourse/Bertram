...
pub async fn fetch_luma_dump(msg: &Message, args: &Args) -> CommandResult<CrashLuma> {
    let file = if msg.attachments.len() > 0 {
        msg.attachments[0].download().await?
    } else {
        reqwest::get(args.current().ok_or("no file given")?)
            .await?
            .bytes()
            .await?
            .into()
...
pub async fn luma(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    let dump = fetch_luma_dump(msg, &args).await?;
...
#[command]
pub async fn stack(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    let dump = fetch_luma_dump(msg, &args).await?;
    ...
    for i in 0..(dump.stack.len()/4) {
        formatted_stack += &format!("{:08x}{}", u32::read_from(&mut stack_slice, LE).unwrap(), if i % 4 == 3 {"\n"} else {" "});
    ...
    msg.reply(ctx, format!("Stack dump (w/endian) (sp = `{:08x}`):```{}```", dump.registers[13], &formatted_stack)).await.unwrap();
    Ok(())
}